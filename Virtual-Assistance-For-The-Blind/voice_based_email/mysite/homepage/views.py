...
def options_view(request):
    ...
    if request.method == 'POST':
        ...
        i = i + str(1)
        while(flag):
            texttospeech("To compose an email say compose. To open Inbox folder say Inbox. To open Sent folder say Sent. To open Trash folder say Trash.To open starred folder say star.To Logout say Logout. Do you want me to repeat?", file + i)
            i = i + str(1)
            say = speechtotext(3)
            if say == 'No' or say == 'no':
                flag = False
        ...
        if act == 'compose':
            ...
            return JsonResponse({'result' : 'star'})
        elif act == 'log out':
            addr = ""
            passwrd = ""
...
def read_mails(mail_list,folder):
    ...
    while flag :
        ...
        if folder == 'inbox' or folder == 'sent':
            texttospeech("If you want to move this email to starred folder say yes or else say no",file+i)
            i=i+str(1)
            say=speechtotext(5)
            say=say.lower()
            if say=='yes' or say=='n':
                try:
                    conn.store(data, '+FLAGS', '(\Flagged)')
                    conn.expunge()
                    texttospeech("The mail has been moved to the starred folder successfully.", file + i)
                    i = i + str(1)
                    print("mail moved to starred folder")
                except:
        ...
        if folder == 'trash':
            ...
            if ans == "yes" or ans=="n":
                try:
                    ...
                    i = i + str(1)
        if folder == 'starred':
            texttospeech("Do you want to remove this mail from starred folder? Say yes or no. ", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            print(ans)
            if ans == "yes" or ans == 'n':
                try:
                    conn.store(data, '-FLAGS', '(\Flagged)')
                    conn.expunge()
                    texttospeech("The mail has been removed from starred folder successfully.", file + i)
                    i = i + str(1)
                    print("mail deleted")
                except:
                    texttospeech("Sorry, could not remove this mail. Please try again later.", file + i)
                    i = i + str(1)
...
def starred_view(request):
    ...
    if request.method == 'POST':
        ...
        conn = imaplib.IMAP4_SSL(imap_url)
        conn.login(addr, passwrd)
        conn.select('"[Gmail]/Starred"')
        result1, data1 = conn.search(None, "ALL")
        mail_list = data1[0].split()
        text = "You have reached your starred folder. You have " + str(len(mail_list)) + " mails in your starred folder. To search a specific email say search.To go back to the menu page say back. To logout say logout."
        texttospeech(text, file + i)
        i = i + str(1)
        flag = True
        while (flag):
            ...
            if act == 'search':
                ...
                emailid = emailid.lower()
                emailid = convert_special_char(emailid)
                search_specific_mail('"[Gmail]/Starred"', 'FROM', emailid, 'starred')
            ...
            texttospeech("If you wish to do anything in the starred folder say yes or else say no.", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            print(ans)
        ...
        return render(request, 'homepage/starred.html')
